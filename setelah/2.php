...
<table  border="1" width="100%">
	...
	$query = 'SELECT`burung`.`nama_burung`,`wilayah`.`nama_wilayah`, `harga`.`Harga` FROM `harga`,`burung`,`wilayah` WHERE `burung`.`id_burung`=`harga`.`id_burung`&&`wilayah`.`id_wilayah`=`harga`.`id_wilayah`&&`harga`.`Harga`=`harga`.`Harga` && `harga`.`id_wilayah`=2 ORDER BY `burung`.`id_burung`,`wilayah`.`id_wilayah`'; // Query untuk menampilkan semua data siswa
