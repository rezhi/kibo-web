...
if($_POST){
	...
	if(!empty($_FILES['images']['tmp_name'])){
		...
		$query = "insert into pakan(judul_artikel, isi_artikel, penulis_artikel, photo_artikel)
		values('$judulartikel', '$isiartikel', '$penulisartikel', '$photoartikel')";
		mysqli_query($conn, $query); 
		
	} else {
		$query = "insert into pakan(judul_artikel, isi_artikel, penulis_artikel)
		values('$judulartikel', '$isiartikel' ,'$penulisartikel')";
		mysqli_query($conn, $query);
		
	}
	header('location:tabel_pakan.php');
	exit;
}
